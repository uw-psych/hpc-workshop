...
`tldr` is a user-friendly supplement to `man` pages that provides practical examples of how to use a command:

``` bash
# Install tldr from the Python Package Index:
pip3.9 install --user tldr
# Show usage examples for ls:
tldr ls
```

<!-- img tldr.png -->

##  `ranger` for easy file system nbavigation
...
- Navigate the file system using the arrow keys 
...
- `hyakalloc` shows the resources available across all the nodes on the cluster -- not just the resources available on a single node
...
This will submit a request for an interactive session to the SLURM scheduler. You may have to wait for resources to become available -- use `squeue` to check the status of your request.
...
To run the Python script inside the container, use the `apptainer run` command:

``` bash
apptainer run workshop-01_latest.sif python
```
...
``` bash
apptainer run workshop-01_latest.sif python single-job.py
```
...
Still not working -- Apptainer can't find the input file. This is because the container is isolated from the /gscratch directory. We need to bind the /gscratch directory to the container, which you can do by setting the environment variable `APPTAINER_BINDPATH`:
...
``` bash
salloc --account escience --partition gpu-a40 --time=1:00:00 --mem=16G --cpus-per-task=6
```
...
``` bash
sbatch --account escience --partition gpu-a40 --mem 16G --cpus-per-task 4 --time 1:00:00 --array=1-4 sbatch.job
```
...
``` bash
scp your-uw-netid@klone.hyak.uw.edu:/gscratch/scrubbed/your-uw-netid/hpc-workshop/workshop-01/output job-output
```
