...
# Get task count and task id from
#   SLURM_ARRAY_TASK_COUNT and SLURM_ARRAY_TASK_ID environment vars:
task_count = int(os.getenv("SLURM_ARRAY_TASK_COUNT", 1))
task_id = int(os.getenv("SLURM_ARRAY_TASK_ID", 1))
min_task_id = int(os.getenv("SLURM_ARRAY_TASK_MIN", 1))
task_index = (task_id - min_task_id) + 1
...
# Run sapa_bootstats:
sapa_bootstats.run(
    input_path, categories=task_categories, n_iter=n_iter, output_dir=output_dir
)
